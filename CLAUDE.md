# MiAyudaTics - Gu铆a de Proyecto

## Descripci贸n y Stack
Sistema de tickets para instituciones educativas centralizando reportes de hardware/software, con r谩pida respuesta.
**Stack:** React 18, Vite, Node/Express, MongoDB.
**Gestor de paquetes:** pnpm estricto.

## ADRs (Architecture Decision Records)
- **ADR-001:** Strict TypeScript Migration (prohibido expl铆citamente el uso de `any`, `allowJs` solo temporal).
- **ADR-002:** Zod para contratos en Runtime en API.
- **ADR-003:** Feature-Based Architecture (`features/auth`, etc.).
- **ADR-004:** Ecosistema Testing: Vitest (Frontend) + Jest (Backend) + Playwright (E2E).
- **ADR-005:** Migraci贸n total a pnpm y auditor铆a estricta de dependencias.

## Definition of Done (Fase 1)
- C贸digo en TS con tipos expl铆citos (sin any).
- Tests unitarios + integraci贸n por m贸dulo.
- Sin errores de lint (ESLint + Prettier).
- Documentaci贸n inline JSDoc donde sea necesario.
- M贸dulo explicable para devs de 1 mes exp.

## Non-goals (Esta fase)
- No redise帽ar DB.
- No cambiar framework (permanece React 18 y Express).
- No microservicios.
- No CI/CD.

## Estado Actual de la Migraci贸n
- **Fase 1 (Setup de Calidad):** En progreso (Tarea 1.0 parcial: Dependencias del Client auditadas).
- **Fase 2:** Pendiente.

## Convenciones de C贸digo
- Orden de Git Hook: 1. `tsc --noEmit` (aborta si falla), 2. `eslint --fix`.
- TS Estricto, uso de Zod para validaci贸n.

## Comando Dev
Frontend: `cd client && pnpm run dev`
Backend: `cd server && pnpm run dev`

## ?? Deuda T閏nica Activa
- allowJs: true ? eliminar al 100% de migraci髇 (Tarea 2.0)
- SKIP_PRE_COMMIT bypass ? eliminar al terminar Tarea 1.3
