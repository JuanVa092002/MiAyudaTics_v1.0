...
const asignarTecnicoSolicitud = async (req, res) => {
  try {
    ...
    const tecnicoAsignado = await usuarioModel.findOne({
      _id: tecnico,
      rol: 'tecnico',
      estado: true,
    })
...
const getSolicitudesAsignadas = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `solicitudes asignadas tecnico ${tecnico.nombre}`, solicitudesAsignadas })
  } catch (error) {
    handleHttpError(res, 'Error al obtener datos')
  }
}
