    ...
    const fetchCaseTypes = async () => {
      try {
        ...
        if (Array.isArray(caseTypesData)) {
          ...
          console.error(
            'La propiedad data de la respuesta de la API no es un array:',
            caseTypesData
          )
          setCaseTypes([])
        }
  ...
  const columns = [
    {
      ...
      cell: row => <div className="break-words overflow-hidden max-w-xs">{row.codigoCaso}</div>,
    },
    {
      name: 'Fecha',
      selector: row => row.fecha,
      cell: row => <div className="break-words overflow-hidden max-w-xs">{row.fecha}</div>,
    },
    {
      name: 'Ambiente',
      ...
      cell: row => (
        <div className="break-words overflow-hidden max-w-xs">
          {row.ambiente?.nombre || 'Sin asignar'}
        </div>
      ),
    },
    {
      ...
      cell: row => <div className="break-words overflow-hidden max-w-xs">{row.descripcion}</div>,
    },
    {
      name: 'Usuario',
      ...
      cell: row => (
        <div className="break-words overflow-hidden max-w-xs">
          {row.usuario?.nombre || 'No disponible'}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: row => <div className="break-words overflow-hidden max-w-xs">{row.estado}</div>,
    },
    {
      name: 'Acciones',
      cell: row => <button onClick={() => openModal(row)}>Solucionar Caso</button>,
    },
  ]
          ...
          {selectedCase && (
            <SolutionModal
              ...
              setSolutionDescription={value =>
                setSelectedCase({ ...selectedCase, solucion: value })
              }
